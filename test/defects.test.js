...
import { createError, errorResponse } from '../src/errors.js';
import { accountInfoFromTokens, parseManualAuthInput } from '../src/oauth.js';
import { preserveAccountRouting, validateRouting } from '../src/routing.js';
import { createProxyServer } from '../src/server.js';
import { usageLines } from '../src/usage-view.js';

const accounts = [{ name: 'a', type: 'apikey', apiKey: 'secret-a' }, { name: 'b', type: 'apikey', apiKey: 'secret-b' }];
const policy = pool => ({ accounts, routing: { defaultPool: 'main', pools: { main: pool } } });
...
test('ordered failover skips disabled first member', () => {
  const manager = new AccountManager([{ ...accounts[0], enabled: false }, accounts[1]], 0.98, policy({ accounts: ['a', 'b'], strategy: 'failover' }).routing);
...
test('empty usage pools display zero instead of NaN', () => {
  const lines = usageLines({ accounts: [], routing: { pools: { empty: { accounts: [] } } } });
  assert.ok(lines.includes(' empty: 0 requests | in 0 | out 0'));
  assert.doesNotMatch(lines.join('\n'), /NaN|undefined/);
});
test('git ignores nested credentials and generated output but keeps examples', { skip: !existsSync('.git') }, () => {
...
for (const name of ['', 'toString']) test(`proxy rejects invalid explicit pool ${JSON.stringify(name)}`, async t => {
  const manager = new AccountManager(accounts);
  const server = createProxyServer(manager, { proxy: { apiKey: 'proxy' } });
  server.listen(0, '127.0.0.1');
  await once(server, 'listening');
  t.after(() => { server.closeAllConnections(); server.close(); });
  const res = await fetch(`http://127.0.0.1:${server.address().port}/responses`, { headers: { 'x-teamcodex-pool': name }, signal: AbortSignal.timeout(2000) });
...
test('disabled display status preserves the underlying auth failure', () => {
  const manager = new AccountManager([{ ...accounts[0], enabled: false }]);
...
test('token refresh is visible without making a disabled account look available', async () => {
  const { accountStatus } = await import('../src/account-status.js');
