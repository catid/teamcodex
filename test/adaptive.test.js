...
import { AdaptiveRouting } from '../src/adaptive-routing.js';
import { validateRouting } from '../src/routing.js';

const accounts = [{ name: 'a', type: 'apikey', apiKey: 'a', weight: 3 }, { name: 'b', type: 'apikey', apiKey: 'b', weight: 1 }];
const routing = { defaultPool: 'main', pools: { main: { accounts: ['a', 'b'], strategy: 'adaptive' } } };
...
test('adaptive configuration preserves pool and quota eligibility', () => {
  ...
  manager.accounts[1].status = 'error';
...
test('adaptive selection shifts traffic but continues probing slower accounts', () => {
  ...
  let now = 0;
  manager.adaptive = new AdaptiveRouting(() => now);
  const slow = manager.adaptive.start(manager.accounts[0]);
  now = 5000;
  slow.observe(false);
  slow.release();
