...
import type { Account, UsageResetStatus } from '@teamcodex/core/accounts';
import type { Config, ResetState } from '@teamcodex/core/config';
...
import { atomicConfigUpdate } from './config.ts';
...
interface SnapshotSource { account: Account; accountId: string | null; credential: string | undefined }
interface Reservation extends ResetState { pendingRequestId: string; lastAttemptAt: string }
interface MonitorOptions {
  fetchFn?: typeof fetch;
  updateConfig?: typeof atomicConfigUpdate;
  now?: () => number;
  wait?: typeof retryDelay;
}
const record = (value: unknown): Record<string, unknown> => isRecord(value) ? value : {};

function enabled(account: Account): boolean { return account.enabled !== false; }
function failed(account: Account): boolean { return account.status === 'error'; }

function autoResetPolicy(config: Config) {
  return {
...
function errorCode(error: unknown): string {
  ...
  return typeof err.code === 'string' && typeof err.message === 'string' && ['USAGE_HTTP_ERROR', 'USAGE_INVALID', 'USAGE_RESPONSE_TOO_LARGE', 'USAGE_RESPONSE_INVALID', 'USAGE_ACCOUNT_MISMATCH'].includes(err?.code)
...
export class UsageResetMonitor {
  ...
  // Provenance binds snapshots to one account and credential, never pool totals.
  private readonly snapshots = new WeakMap<UsageSnapshot, SnapshotSource>();
  ...
  start() {
    ...
    const check = () => this.check().catch(err => console.error(errorMessage('USAGE_MONITOR_FAILED', { message: (err instanceof Error ? err.message : String(err)) })));
  ...
  async request(account: Account, path: string, body?: { redeem_request_id: string }): Promise<unknown> {
    const credential = account.credential;
    const accountId = account.accountId;
    for (let attempt = 0; ; attempt++) {
      this.controller.signal.throwIfAborted();
      if (!enabled(account) || !this.manager.accounts.includes(account) || account.accountId !== accountId || account.credential !== credential) {
      ...
      try {
        ...
        const status = /^http_(\d{3})$/.exec((err instanceof Error ? err.message : String(err)))?.[1];
  ...
  async readUsage(account: Account): Promise<UsageSnapshot> {
    const accountId = account.accountId;
    const credential = account.credential;
    ...
    if (isRecord(payload) && payload.account_id != null && payload.account_id !== accountId) throw createError('USAGE_ACCOUNT_MISMATCH');
  ...
  async reserve(account: Account, snapshot: UsageSnapshot | null): Promise<Reservation | null> {
    ...
    await this.updateConfig(async config => {
      ...
      if (!entry || entry.type !== 'chatgpt' || entry.enabled === false || !enabled(account)) return;
      ...
      if (accountId !== identity.accountId || diskCredential !== credential ||
          !this.manager.accounts.includes(account) || !enabled(account) ||
      ...
      if (!state.pendingRequestId) {
        ...
        if (!snapshot || now < snapshot.fetchedAt || now - snapshot.fetchedAt > MAX_SNAPSHOT_AGE_MS ||
            !((snapshot.resetCreditsAvailable ?? 0) > 0) || snapshot.utilization < policy.threshold) return;
      }
      ...
      const startedAt = state.pendingRequestId ? state.lastStartedAt || state.lastAttemptAt : new Date(now).toISOString();
      reservation = {
        ...state,
  ...
  publicState(state: ResetState): Partial<UsageResetStatus> {
    const started = Date.parse(state.lastStartedAt || state.lastAttemptAt || '');
  ...
  async redeem(account: Account, reservation: Reservation): Promise<void> {
    const accountId = account.accountId;
    const credential = account.credential;
    ...
    let settled = false;
    try {
      const response = record(await this.request(account, RESET_PATH, { redeem_request_id: reservation.pendingRequestId }));
      if (response.code === 'no_credit' || response.code === 'nothing_to_reset') {
        result = response.code;
        settled = true;
        if (result === 'no_credit' && account.accountId === accountId) account.usageReset.availableCredits = 0;
      } else if (response.code === 'reset' || response.code === 'already_redeemed') {
        try {
    ...
    if (!settled && this.timer && !this.pendingTimer && !this.controller.signal.aborted) {
      this.pendingTimer = setTimeout(() => {
        ...
        void this.check().catch(err => console.error(errorMessage('USAGE_PENDING_CHECK_FAILED', { message: (err instanceof Error ? err.message : String(err)) })));
      }, PENDING_RETRY_MS);
