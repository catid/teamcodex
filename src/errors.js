...
/** @typedef {Record<string, string | number>} ErrorParameters */

/**
 * Format a registered diagnostic. Keep dynamic values out of public responses
 * unless the caller explicitly chooses to expose them.
 * @param {ErrorCode} code
 * @param {ErrorParameters} [parameters]
 * @returns {string}
 */
export function errorMessage(code, parameters = {}) {
  const definition = ERROR_CODES[code];
  if (!definition) throw new TypeError(`Unknown error code: ${code}`);
  return definition.message.replace(/\{(\w+)\}/g, (_, key) => {
    if (!Object.hasOwn(parameters, key)) throw new TypeError(`Missing error parameter: ${key}`);
    return String(parameters[key]);
  });
}

/** Application error; native/provider errors can be retained as its cause. */
 ...
 * @returns {AppError}
 */
export function createError(code, parameters = {}, options = {}) {
  return new AppError(code, parameters, options);
}

/**
 * Serialize only the public contract, never a stack or cause.
 * @param {ErrorCode} code
 * @param {ErrorParameters} [parameters]
 * @returns {{error: {code: ErrorCode, opcode: number, message: string, type?: string}}}
 */
export function errorResponse(code, parameters = {}) {
