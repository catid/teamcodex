import { panel } from './tui-panels.js';
import { bold, cyan, dim } from './tui-style.js';

/** @param {string} verificationUrl @param {string} userCode @param {number} width @returns {string[]} */
export function devicePrompt(verificationUrl, userCode, width) {
...
/** @param {string} url @param {number} width @returns {string[]} */
export function browserPrompt(url, width) {
  const chunks = [];
