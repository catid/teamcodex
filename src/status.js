...
const tokens = value => (value?.inputTokens || 0) + (value?.outputTokens || 0);
...
export const displayWidth = value => [...clean(value)].reduce((sum, char) => sum + cellWidth(char), 0);
...
const age = (value, now) => finite(time(value)) ? `${duration(now - time(value))} ago` : 'never';
const until = (value, now) => !finite(time(value)) ? 'time unknown' : time(value) <= now ? 'due now' : `in ${duration(time(value) - now)}`;
const pct = value => finite(value) ? `${(value * 100).toFixed(1)}%` : 'unknown';
const ratio = (remaining, limit) => finite(remaining) && finite(limit) && limit > 0 ? 1 - remaining / limit : null;

function windows(account) {
  const q = account.quota || {};
  const label = (minutes, fallback) => minutes === 10080 ? 'Weekly' : finite(minutes) && minutes > 0 ? duration(minutes * 60000) : fallback;
...
export function renderStatus(data, { columns = 110, color = false, compact: brief = false, now = Date.now() } = {}) {
  ...
  const paint = (text, tone) => color && codes[tone] ? `\x1b[${codes[tone]}m${text}\x1b[0m` : text;
  function line(value = '', tone, indent = 2) {
    const text = clean(value);
  ...
  if (policy) line(`Usage poll every ${duration(policy.pollIntervalSeconds * 1000)}  ·  ${data.usagePolling?.running ? 'checking accounts now' : `last finished ${age(data.usagePolling?.lastCompletedAt, now)}`}`, 'dim');
  rule();
  ...
  if (brief && width >= 85) {
    ...
    accounts.forEach((a, i) => {
      const ws = windows(a);
      line(`${fit(`${a.name === data.currentAccount ? '▸' : ' '} ${a.name}`, nameWidth)}  ${fit(states[i].label, 13)} ${fit(pct(ws[0]?.value), 9)} ${fit(pct(ws[1]?.value), 9)} ${fit(a.type === 'chatgpt' ? a.usageReset?.availableCredits ?? '?' : 'n/a', 7)} ${compact(a.totals ? tokens(a.totals) : (a.usage?.totalInputTokens || 0) + (a.usage?.totalOutputTokens || 0))}`, a.name === data.currentAccount ? 'cyan' : undefined);
    });
