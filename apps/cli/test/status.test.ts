...
import { displayWidth,renderStatus } from '../src/commands/status.ts';
import { normalizeStatus } from '../src/commands/status-data.ts';
...
function fixture() {
  ...
  const identity: AccountConfig = { name: account.name, type: 'chatgpt', accountId: account.accountId };
  const stats = new UsageStats(null, { now: () => now });
  stats.recordTokens(identity, 10000, 2000, 5000);
  const entry = normalizeStatus({ accounts: [{ ...account, totals: stats.account(identity) }] }).accounts[0];
  assert.ok(entry);
  return { accounts: [entry] satisfies [typeof entry], currentAccount: account.name, rotationOrder: [account.name], switchThreshold: 0.98,
...
for (const [status, label] of [['disabled', 'Disabled'], ['refreshing', 'Refreshing']] as const) {
...
test('expired cooldowns, pending resets and missing credentials are distinguishable', () => {
  ...
  data.accounts[0].usageReset = { availableCredits: null, checkedAt: null, lastResult: null, pending: true, retryAt: new Date(now + 60000).toISOString() };
...
test('JSON CLI emits only parseable data and sends authentication; errors fail clearly', async () => {
  ...
  const source = `import {statusCommand} from ${JSON.stringify(new URL('../src/commands/status.ts', import.meta.url).href)};
    await statusCommand({proxy:{apiKey:'fake-secret'}}, process.argv.slice(1));`;
  const run = (...args: string[]) => promisify(execFile)(process.execPath, ['--input-type=module', '-e', source, '--', ...args], {
    env: { ...process.env, TEAMCODEX_SERVER_URL: `http://127.0.0.1:${address.port}` }, timeout: 10000,
  });
  const result = await run('--json');
  assert.deepEqual(JSON.parse(result.stdout), JSON.parse(JSON.stringify(data)));
