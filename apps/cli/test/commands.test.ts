...
async function fixture() {
  ...
  const env: NodeJS.ProcessEnv & { TEAMCODEX_CONFIG: string } = { ...process.env, TEAMCODEX_CONFIG: configPath, CODEX_HOME: dir, PATH: `${bin}:${process.env.PATH}` };
  await writeFile(join(bin, 'codex'), `#!/usr/bin/env bun
...
test('native run passes literal arguments and the proxy key without changing Codex auth', async () => {
  const f = await fixture();
  const auth = '{"tokens":{"access_token":"unchanged"}}';
  await writeFile(join(f.dir, 'auth.json'), auth);
  const { stdout } = await exec(process.execPath, [join(root, 'apps/cli/src/index.ts'), 'run', '--safe', 'exec', '-c', 'model_reasoning_effort=low', 'spaces $() `literal`'], { env: f.env, cwd: f.dir });
...
test('env output is safely quoted for Bash and keeps the API key out of Codex arguments', async () => {
  const f = await fixture();
  ...
  const { stdout } = await exec(process.execPath, [join(root, 'apps/cli/src/index.ts'), 'env'], { env: f.env });
...
test('Docker launcher works through a symlink with spaces and preserves host cwd', async () => {
  const f = await fixture();
  // Mock only the Docker transport; the actual CLI emits the launch protocol.
  await writeFile(join(f.bin, 'docker'), `#!/usr/bin/env bun
import { spawnSync } from 'node:child_process';
import { readFileSync } from 'node:fs';
const args = process.argv.slice(2);
if (args.includes('run') && !args.includes('--interactive=false')) readFileSync(0);
if (args.includes('env') && args.includes('--null')) {
  const result = spawnSync(process.execPath, [${JSON.stringify(join(root, 'apps/cli/src/index.ts'))}, 'env', '--null'], {stdio:'inherit'});
...
async function staleGroupFixture({ membership = 'stale', unavailable = false }: { membership?: string; unavailable?: boolean } = {}) {
  const f = await fixture();
  await writeFile(join(f.bin, 'package.json'), '{"type":"module"}');
  const scripts = {
...
process.exit(result.status ?? 1);`,
  ...
  for (const [name, script] of Object.entries(scripts)) {
    await writeFile(join(f.bin, name), `#!/usr/bin/env bun\n${script}\n`, { mode: 0o755 });
  }
...
test('old tmux Docker groups recover while preserving literal arguments, cwd, and proxy settings', async () => {
  const f = await staleGroupFixture();
...
test('unavailable Docker retries stale membership only once and reports the original error', async () => {
  const f = await staleGroupFixture( { unavailable: true });
  await assert.rejects(exec('/bin/bash', [join(root, 'teamcodex.sh'), 'ps'], { env: f.env }), err => {
    assert.ok(isRecord(err));
...
for (const membership of ['absent', 'current']) {
  test(`Docker failure does not invoke sg when group membership is ${membership}`, async () => {
    const f = await staleGroupFixture( { membership });
