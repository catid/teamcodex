...
import { TUI } from '@teamcodex/cli/tui';
...
test('TUI accepts pasted API keys and masks the input display', () => {
  const tui = new TUI({ accountManager: new AccountManager([]), config: createDefaultConfig(), saveConfig: async () => {}, syncAccounts: async () => {} });
  tui.mode = 'input';
...
test('account reset credits distinguish known zero, unknown and unsupported accounts', () => {
  ...
  const tui = new TUI({ accountManager, config: createDefaultConfig(), saveConfig: async () => {}, syncAccounts: async () => {} });
  assert.match(tui._renderAcct(0, 8, true), /Resets .*\?/);
  const account = accountManager.accounts[0];
  assert.ok(account);
  account.usageReset.availableCredits = 0;
  assert.match(tui._renderAcct(0, 8, true), /Resets .*0/);
  account.usageReset.availableCredits = 3;
  assert.match(tui._renderAcct(0, 8, true), /Resets .*3/);
  assert.match(tui._renderAcct(1, 8, true), /Resets .*—/);
});

test('device OAuth exchanges the provider PKCE verifier and rejects missing verifiers', async () => {
  const { deviceCodeLogin } = await import('@teamcodex/proxy/auth/device');
  const original = globalThis.fetch;
  afterEach(() => { globalThis.fetch = original; });
  const verifier = 'v'.repeat(64);
  let missing = false;
  const requests: { url: Parameters<typeof fetch>[0]; options: RequestInit | undefined }[] = [];
  globalThis.fetch = Object.assign(async (url: Parameters<typeof fetch>[0], options?: RequestInit) => {
    requests.push({ url, options });
    if (String(url).endsWith('/usercode')) return Response.json({ device_auth_id: 'device-id', user_code: 'ABCD-EFGH', interval: 5 });
    if (String(url).endsWith('/deviceauth/token')) return Response.json({ authorization_code: 'code', ...(missing ? {} : { code_verifier: verifier }) });
  ...
  assert.ok(exchange?.options?.body instanceof URLSearchParams);
...
test('browser OAuth binds the S256 challenge, callback state and token verifier', async () => {
  const { createHash } = await import('node:crypto');
  const { loginOAuth } = await import('@teamcodex/cli/oauth');
  const original = globalThis.fetch;
  afterEach(() => { globalThis.fetch = original; });
