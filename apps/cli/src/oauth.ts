...
import type { AuthCode } from '@teamcodex/proxy/auth/callback';
...
import { browserPrompt } from './tui/login.ts';
 ...
 */
export async function loginOAuth({ onAuthorize }: { onAuthorize?: (url: string) => void | Promise<void> } = {}): Promise<Credentials> {
  // Generate PKCE
 ...
 */
function raceWithStdinCode(callbackPromise: Promise<AuthCode>, expectedState: string): Promise<AuthCode> {
  ...
  return new Promise<AuthCode>((resolve, reject) => {
    ...
    const settle = <T>(fn: (value: T) => void, val: T): void => {
      if (settled) return;
      settled = true;
      rl.close();
      fn(val);
