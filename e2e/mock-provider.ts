...
const requests: { method: string | undefined; path: string | undefined; headers: http.IncomingHttpHeaders; body: unknown }[] = [];
let mode = 'success';
let redeemed = false;
...
http.createServer(async (req, res) => {
  ...
  if (req.url === '/control' && req.method === 'POST') {
    if (typeof body.mode !== 'string') { res.writeHead(400); res.end('{}'); return; }
    mode = body.mode;
    requests.length = 0;
    res.end('{}');
    return;
