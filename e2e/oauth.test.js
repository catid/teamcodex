...
for (const scenario of ['browser', 'device', 'wrong-state', 'invalid-token', 'missing-verifier', 'api-key']) {
  test(`offline CLI authentication: ${scenario}`, { timeout: 20_000 }, async t => {
    ...
    t.after(() => { issuer.closeAllConnections(); issuer.close(); });
    ...
    const child = spawn(process.execPath, ['--import', resolve('e2e/oauth-transport.js'), resolve('src/index.js'), 'login', device ? '--device-auth' : scenario === 'api-key' ? '--api' : '--browser', '--name', 'mock'], {
