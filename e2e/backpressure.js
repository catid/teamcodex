...
const request = (input, pool = 'main', signal = AbortSignal.timeout(10_000)) => fetch(`${base}/v1/responses`, {
...
try {
  let arrived = false;
  for (let i = 0; i < 100; i++) {
    const calls = await (await fetch('http://mock:8080/requests')).json();
    if (calls.some(call => call.body?.input === 'hold')) { arrived = true; break; }
    await delay(20);
  }
