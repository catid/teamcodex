...
test('Docker lifecycle with isolated mock providers', async () => {
  ...
  const compose = (...args: string[]) => docker(['compose', '-p', project, '-f', file, ...args], env);
  afterEach(async () => {
  ...
  const config = parseObject(await docker(['compose', '-f', 'compose.yaml', 'config', '--format', 'json'], env));
  const services = object(config.services);
  const service = object(services.teamcodex);
  service.image = image;
  delete service.build;
  service.ports = [{ target: 1456, published: '0', host_ip: '127.0.0.1' }];
  object(service.environment).BUN_OPTIONS = '--preload=/app/e2e/redirect-auth.ts';
  array(service.volumes).push({ type: 'bind', source: join(root, 'e2e'), target: '/app/e2e', read_only: true });
  object(service.healthcheck).interval = '1s';
  services.mock = { image, entrypoint: ['bun', '/app/e2e/mock-provider.ts'], user: service.user, read_only: true, cap_drop: ['ALL'], security_opt: ['no-new-privileges:true'], volumes: [{ type: 'bind', source: join(root, 'e2e'), target: '/app/e2e', read_only: true }], ports: [{ target: 8080, published: '0', host_ip: '127.0.0.1' }], networks: ['default'] };
  ...
  const cli = (...args: string[]) => compose('run', '--rm', '--no-deps', '-T', '-e', 'TEAMCODEX_SERVER_URL=http://teamcodex:1456', 'teamcodex', ...args);
  await step('initialization and invalid configuration rejection', async () => {
    await cli('init');
    const created = await readConfig(configPath);
    ...
    await assert.rejects(cli('serve'), error => {
      assert.ok(isRecord(error) && typeof error.stderr === 'string');
      assert.match(error.stderr, /CONFIG_PORT_INVALID/);
      return true;
    });
  });
  await step('permission failures preserve config and recover after repair', async () => {
  ...
  const fetch = async (url: string, options: RequestInit = {}) => {
    ...
    const data = parseObject(await compose('exec', '-T', 'mock', 'bun', '--input-type=module', '-e', script));
    assert.ok(typeof data.status === 'number');
    const headers = new Headers();
    for (const pair of array(data.headers)) { const entry = array(pair); headers.append(string(entry[0]), string(entry[1])); }
    return new Response(string(data.body), { status: data.status, headers });
  };
  const headers = { authorization: 'Bearer mock-proxy-key', 'content-type': 'application/json' };
  const status = async () => {
    const data = object(await (await fetch(`${base}/teamcodex/status`, { headers })).json());
    return { accounts: array(data.accounts).map(value => { const account = object(value); return { usage: object(account.usage), quota: object(account.quota), usageReset: object(account.usageReset) }; }) };
  };
  const captured = async () => {
    const value: unknown = await (await fetch(`${mock}/requests`)).json();
    assert.ok(Array.isArray(value));
    return value.map((item: unknown) => {
      assert.ok(isRecord(item) && typeof item.path === 'string' && typeof item.method === 'string' && isRecord(item.headers));
      return { path: item.path, method: item.method, headers: item.headers, body: isRecord(item.body) ? item.body : null };
    });
  };
  const mode = (value: string) => fetch(`${mock}/control`, { method: 'POST', headers, body: JSON.stringify({ mode: value }) });
  const request = (body: unknown) => fetch(`${base}/backend-api/codex/responses?e2e=1`, { method: 'POST', headers: { ...headers, 'chatgpt-account-id': 'must-not-leak' }, body: JSON.stringify(body) });
  const restore = async () => {
    await compose('stop', 'teamcodex');
    await writeFile(configPath, JSON.stringify(initial));
    await compose('up', '-d', '--wait', 'teamcodex');
  };
  await step('image, health, permissions, authentication and CLI status', async () => {
    const id = await compose('ps', '-q', 'teamcodex');
    const container = object(array(JSON.parse(await docker(['inspect', id])))[0]);
    const host = object(container.HostConfig);
    assert.equal(object(object(container.State).Health).Status, 'healthy');
    assert.equal(host.ReadonlyRootfs, true);
    assert.deepEqual(host.CapDrop, ['ALL']);
    assert.ok(array(host.SecurityOpt).includes('no-new-privileges:true'));
    assert.notEqual(string(object(container.Config).User).split(':')[0], '0');
    const checks = parseObject(await compose('exec', '-T', 'teamcodex', 'bun', '--input-type=module', '-e', `import fs from 'node:fs';let denied=false;try{fs.writeFileSync('/app/forbidden','x')}catch(e){denied=['EROFS','EACCES'].includes(e.code)}fs.writeFileSync('/tmp/probe','ok');console.log(JSON.stringify({denied,uid:process.getuid?.()}))`));
    ...
    assert.equal(object(object(await unauthorized.json()).error).code, 'INVALID_PROXY_KEY');
    assert.match(await cli('status'), /first/);
    assert.match(await cli('accounts'), /second/);
  });
  await step('runtime isolation, wrong identity and mount permissions', async () => {
    const probe = parseObject(await compose('exec', '-T', 'teamcodex', 'bun', '--input-type=module', '-e', `
    ...
    `));
    assert.match(string(probe.status), /NoNewPrivs:\s+1/);
    assert.match(string(probe.status), /CapEff:\s+0+\n/);
    assert.equal(probe.gid, process.getgid?.());
    assert.equal(probe.config, process.getuid?.());
    await assert.rejects(compose('exec', '-T', '--user', '65534:65534', 'teamcodex', 'bun', 'apps/cli/src/index.ts', 'accounts'), error => {
      assert.ok(isRecord(error) && typeof error.stderr === 'string');
      assert.match(error.stderr, /EACCES/);
      return true;
    });
    ...
    assert.equal(object(host.RestartPolicy).Name, 'unless-stopped');
    assert.equal(object(array(object(host.PortBindings)['1456/tcp'])[0]).HostIp, '127.0.0.1');
    assert.ok(object(host.Tmpfs)['/tmp'] !== undefined);
  });
  await step('healthcheck detects broken config and shutdown releases the listener', async () => {
    const before = await readFile(configPath, 'utf8');
    try {
      await writeFile(configPath, '{broken');
      await assert.rejects(compose('exec', '-T', 'teamcodex', 'bun', 'packages/proxy/src/healthcheck.ts'));
    } finally { await writeFile(configPath, before); }
    await compose('exec', '-T', 'teamcodex', 'bun', 'packages/proxy/src/healthcheck.ts');
    const id = await compose('ps', '-q', 'teamcodex');
    await compose('stop', '--timeout', '10', 'teamcodex');
    const stopped = object(object(array(JSON.parse(await docker(['inspect', id])))[0]).State);
    assert.equal(stopped.Running, false);
    assert.equal(stopped.OOMKilled, false);
    assert.notEqual(stopped.ExitCode, 137);
    assert.equal(await readFile(configPath, 'utf8'), before);
    await compose('up', '-d', '--wait', 'teamcodex');
    assert.equal((await status()).accounts.length, 2);
  });
  await step('JSON and SSE requests preserve payloads and account usage', async () => {
    const body = { model: 'mock-model', input: 'hello', stream: false };
    assert.equal(object(await (await request(body)).json()).output, 'hello');
  ...
  await step('weighted pools distribute requests and strip routing headers', async () => {
    await compose('stop', 'teamcodex');
  ...
  await step('pool saturation, cancellation and slow SSE clients recover capacity', async () => {
    await compose('stop', 'teamcodex');
    await writeFile(configPath, JSON.stringify({
    ...
    await compose('up', '-d', '--wait', 'teamcodex');
    await mode('success');
    await compose('exec', '-T', 'mock', 'bun', '/app/e2e/backpressure.ts');
  ...
  for (const scenario of ['rotate', 'reject', 'all429']) {
    await step(`${scenario}: retries, rotation and error contract`, async () => {
      await restore();
      await mode(scenario);
      ...
      if (scenario === 'all429') {
        assert.equal(object(result.error).code, 'ACCOUNTS_EXHAUSTED');
  ...
  await step('import, OAuth refresh, reset credits, persistence and removal', async () => {
    await restore();
    await mode('success');
    ...
    await compose('stop', 'teamcodex');
    const disk = await readConfig(configPath);
    assert.ok(disk.routing?.pools.main);
    ...
    assert.ok(redemption?.body && typeof redemption.body.redeem_request_id === 'string');
    ...
    assert.equal(object(parseObject(await readFile(join(authDir, 'auth.json'), 'utf8')).tokens).refresh_token, 'refreshed-token');
    assert.equal((await stat(configPath)).mode & 0o777, 0o600);
    ...
    await compose('stop', 'teamcodex');
    await cli('reset');
    const reset = await readConfig(configPath);
...
async function step(name: string, run: () => Promise<void>) {
  ...
  catch (cause) { throw new Error(`Docker scenario failed: ${name}`, { cause }); }
}
